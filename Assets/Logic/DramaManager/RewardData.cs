...
namespace DramaPack{
	public class RewardData : StringData {
		...
		[HideInInspector]public Item chosenItem{
			get{
				if(_chosenItem == null) _chosenItem = new Item(rewardItems[(int)Random.Range(0,rewardItems.Length)],Mathf.Clamp (DramaManager.progression+ HubManager.shop.level * 7,HubManager.shop.level * 6, HubManager.shop.level * 18));
				return _chosenItem;
			}
			set{
				_chosenItem = value;
